...
def build_adjacency_lists(categories, customers, employees, orders, order_details, products, shippers, suppliers):
    ...
    adjacency_lists = build_node_list(
        categories, "categoryID", "categoryName", "description")
    adjacency_lists += build_node_list(customers, "customerID",
                                       "contactName", "country#region#city#address", )
    adjacency_lists += build_node_list(shippers,
                                       "shipperID", "companyName", "phone")
    adjacency_lists += build_node_list(suppliers, "supplierID",
                                       "SUPPLIER", "country#region#city#address", )
    adjacency_lists += build_node_list(employees,
                                       "employeeID", "reportsTo", "hireDate")
    adjacency_lists += build_node_list(orders, "orderID", "ORDER", "orderDate")
    adjacency_lists += build_node_list(products,
                                       "productID", "PRODUCT", "discontinued")
    adjacency_lists += build_node_list(order_details,
...
def handler():
    ...
    parser.add_argument(
        "--setup", help="create DynamoDB table before loading data", action='store_true')
    parser.add_argument(
        "--teardown", help="delete DynamoDB table", action='store_true')
    ...
    if args.teardown:
        ...
        table_data = build_adjacency_lists(
